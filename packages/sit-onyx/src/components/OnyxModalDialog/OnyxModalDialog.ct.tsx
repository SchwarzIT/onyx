...
test("Screenshot test (left)", async ({ mount, page, makeAxeBuilder }) => {
  await page.setViewportSize({ width: ONYX_BREAKPOINTS.sm + 1, height: 512 });

  // ARRANGE
  await mount(
    <TestWrapperCt alignment="left">
      <template v-slot:footer>
        <OnyxBottomBar>
          <OnyxButton label="Button" color="neutral" mode="plain" />
          <OnyxButton label="Button" />
  ...
  // ASSERT
  await expect(page).toHaveScreenshot("alignment-left.png");
...
test("Screenshot test (right)", async ({ mount, page, makeAxeBuilder }) => {
  await page.setViewportSize({ width: ONYX_BREAKPOINTS.sm + 1, height: 512 });

  // ARRANGE
  await mount(
    <TestWrapperCt alignment="right">
      <template v-slot:footer>
        <OnyxBottomBar>
          <OnyxButton label="Button" color="neutral" mode="plain" />
          <OnyxButton label="Button" />
  ...
  // ASSERT
  await expect(page).toHaveScreenshot("alignment-right.png");
