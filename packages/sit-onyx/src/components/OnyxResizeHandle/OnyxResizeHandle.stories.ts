 ...
 *
 * Usage: - drag border: resizes the component accordingly - press "Escape" (while dragging):
 * Cancels the current resizing and reverts the width back to the previous value - double mouse
 * click: Trigger auto-sizing (depends on the usage/implementation). Usually should set the
 * component to some use-case specific value or reset the width back to the original one.
 */
