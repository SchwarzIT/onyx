...
export type OnyxStepperProps = Omit<
  ...
  "errorMessages" | "withCounter" | "maxlength" | "label"
> &
  Pick<OnyxFormElementV2Props, "label"> & {
    ...
    stepSize?: number;
    /**
     * Number of decimal places to show. Can also be negative. Value will be rounded if needed to
     * match the specified precision.
     *
     * @example
     *   For `precision=2` with `modelValue=5`, "5.00" will be displayed.
     *
     * @example
     *   For `precision=-2` with `modelValue=60`, "100" will be displayed.
     *
     * @example
     *   For `precision=0`, only full numbers without decimals will be displayed.
     */
    precision?: number;
    /**
     ...
     *
     * @example
     *   For `validStepSize` 0.01, only multiples of 0.01 are valid (useful for currencies)
     *
     * @example
     *   For `stepSize=4` `validStepSize=2`, only even numbers are valid and the user can adjust the value by 4 when clicking the +/- button.
     */
    validStepSize?: number;
    /**
     * Specify how to provide automated assistance in filling out the input. Some autocomplete
     * values might required specific browser permissions to be allowed by the user. Also browsers
     * might require a `name` property.
     *
