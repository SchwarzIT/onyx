...
export type OnyxPageLayoutProps = Partial<SkeletonProvidedProp> & {
  ...
  /**
   * If `true`, the default responsive page content padding will be removed. You can use the
   * `onyx-grid-layout` CSS class on your own which is used internally for the padding. Useful when
   * e.g. implementing a page with a full width hero image at the top, followed by content below.
   */
  noPadding?: boolean;
};
