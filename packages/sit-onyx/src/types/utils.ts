...
/**
 * Merges two types destructively, right to left: All keys from `Source` are kept as is and only the
 * extraneous keys from `Target` that are not present in `Source` are copied over.
 *
 * @example
 *   ```ts
 ...
 *
 * This is similar to the inferred type of a destructuring merge: `{ ...{ a: 1, b: "str" }, ...{ b:
 * true }, ...{ b: 2 } }`.
 *
 * @example
 *   ```ts
