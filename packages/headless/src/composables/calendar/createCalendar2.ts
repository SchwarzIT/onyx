 ...
 */
export const _unstableCreateCalendar2 = createBuilder((options: CreateCalendarOptions) => {
  ...
  const handleKeyNavigation = async (event: KeyboardEvent) => {
    ...
    const getNewFocusDateByDiff = (diff: number) => {
    ...
    switch (event.key) {
      ...
      case "PageUp":
        newDate = getNewFocusDateByDiff(-(event.shiftKey ? 12 : 1));
        break;
      case "PageDown":
        newDate = getNewFocusDateByDiff(event.shiftKey ? 12 : 1);
        break;
    ...
    switch (selectionMode) {
      ...
      case "range": {
        ...
        if (currentRange?.start && currentRange.end) {
          ...
          // TODO: check if end date is smaller than start date
          // TODO: check if new date is equal to start date
          newRange = {
            start: currentRange?.start ?? new Date(date),
