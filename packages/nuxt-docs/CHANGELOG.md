...
- 2a8ce5f: refactor(useCollection): require `collection` name to be passed in as parameter

  Other changes
  - feat!: support passing a `path` option to set which collection path should be queried. By default, the current path will be used instead of the `slug` route parameter
