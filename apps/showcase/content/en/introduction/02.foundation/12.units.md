---
title: Units
description: Discover how our systematic approach to margins, paddings, and border radii establishes the foundation for a balanced, accessible, and unified user experience across all digital products.
---

# Units

Discover how our systematic approach to margins, paddings, and border radii establishes the foundation for a balanced, accessible, and unified user experience across all digital products.

## Base system

::info-card{color="info" headline="Usage"}
Always use the provided variables rather than hardcoded pixel values to maintain system integrity.
::

A unit system simplifies component layouts. Our design system utilizes a **4px base unit**, which scales through linear multiplication. This establishes the foundation for responsive design and a customizable density system, improving overall product quality and accessibility.

## Relative Units (rem)

To ensure accessibility and fluid responsiveness, our design system is developed using `rem`.

- Logic: Measurements are relative to the browser's default font size (typically 16px).
- Conversion: 24px = 1.5rem
- Benefit: Using `rem` allows the layout to scale automatically if a user adjusts their browser’s font settings, ensuring a compliant and flexible interface.

## Spacing

Consistent spacing is critical for establishing visual hierarchy and professional aesthetics.

## Border Radius

The choice of border radius significantly impacts the interface appearance. Border radius variables allow you to customize this visual foundation easily.

The base value for component border radii is 4px, which is used for elements like buttons. When elements are nested inside other components, the outer component may require a larger border radius to maintain visual harmony.
