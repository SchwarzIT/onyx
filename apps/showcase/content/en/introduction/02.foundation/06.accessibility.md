---
title: Accessibility
description: Web accessibility guidelines and laws are crucial for ensuring that the digital content is inclusive and accessible to individuals with disabilities.
---

# Accessibility

Find a comprehensive overview of how our design system achieves WCAG 2.2 (Level AA) compliance through robust support for screenreaders, predictable keyboard navigation and more.

## Focus

Accessibility is categorized into three cumulative levels. We focus on the industry standard for enterprise applications:

- Level A (Minimum): Addresses the most critical barriers—such as basic keyboard operability—without which navigation would be impossible for many users.
- Level AA (Standard): Our target standard. This level optimizes readability and interaction reliability for the general public and meets the legal requirements for digital products.
- Level AAA (Excellence): The highest tier with specialized requirements, such as sign language interpretation, typically relevant for specific government agencies or dedicated target groups.

Supporting Assistive Technologies (AT)
For us, accessibility is more than just a checklist; it is an architectural priority. Our design system is engineered to harmonize with assistive technologies (AT), maximizing their functionality and effectiveness.

Interaction Models
Users interact with digital content in diverse ways. We ensure these workflows are supported by a robust technical foundation:

1. Screenreaders (Visual Accessibility)
   We utilize strict semantic HTML and precise ARIA labels.

Function: Software such as TalkBack, NVDA, or JAWS converts screen content into synthetic speech or Braille.

Precision: Beyond standard text, we transmit critical states (e.g., checked/expanded) and the context of icons that would otherwise only be visually encoded.

2. Keyboards & Alternative Input Devices
   Many users navigate linearly rather than using a pointer.

Function: Users move through the interface by jumping from element to element (Tab order).

Precision: We guarantee a logical, predictable focus order. This ensures a seamless experience whether using a keyboard, trackball, joystick, or D-pad.

3. Switch Access
   We optimize for users with significant motor impairments.

Function: Navigation via step-by-step scanning of on-screen elements.

Precision: Every interactive element is designed to be precisely targeted and triggered using minimal input methods, such as a single-switch system.
