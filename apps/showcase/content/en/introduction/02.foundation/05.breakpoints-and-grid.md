...
<image-card src="/images/grid-visualization.svg" alt="Visualization of the grid system using columns, gutters and margins"></image-card>
...
<steps>
  ...
  Breakpoints specific grid spans are usually combined with one default `onyx-grid-span-<number>` class. See the [breakpoints](#breakpoints) below for an overview of the available breakpoints.


  ```vue [Breakpoint specific grid span]
  <template>
    <div class="onyx-grid">
  ...
  #default
  Our grid system is build using [CSS container queries](https://developer.mozilla.org/en-US/docs/Web/CSS/Guides/Containment/Container_queries) which means that each grid refers to its nearest container to evaluate the current breakpoint.

  When using our [OnyxPageLayout](https://storybook.onyx.schwarz/?path=/docs/layout-pagelayout--docs), [OnyxAppLayout](https://storybook.onyx.schwarz/?path=/docs/layout-applayout--docs) or [OnyxSidebar](https://storybook.onyx.schwarz/?path=/docs/navigation-sidebar--docs) component, the container is automatically defined.

  You can define a custom container by setting the [container-type](https://developer.mozilla.org/en-US/docs/Web/CSS/Reference/Properties/container-type) CSS property on any element. This is particularly useful when e.g. using the grid inside the [OnyxModal](https://storybook.onyx.schwarz/?path=/story/feedback-modal--grid) or custom components so the actual component width is used to determine the number of columns etc.
...
The outer grid margin is already applied by our [OnyxPageLayout](https://storybook.onyx.schwarz/?path=/docs/layout-pagelayout--docs) component. If you still need to implement it manually, simply use the `.onyx-grid-layout` CSS class:
...
<steps>
  ...
  ```scss [Apply styles for screen breakpoint]
  ...
  ```scss [Apply styles for container breakpoint]
  @use "sit-onyx/breakpoints.scss";
...
#ux
Using individual breakpoints directly is not relevant for Figma designs. Instead, refer to the [grid usage](#usage) for how to apply grids in Figma.
::
